...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "cmp.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "cmpd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the CMP and zCMP money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 29000, 30002));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in CMP/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in CMP/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, cmp, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    if (GetBoolArg("-help-debug", false)) {
        ...
        strUsage += HelpMessageOpt("-maxsigcachesize=<n>", strprintf(_("Limit size of signature cache to <n> entries (default: %u)"), 1000));
    }
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in CMP/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Cmp specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:29000"));
    ...
    strUsage += HelpMessageOpt("-backupzcmp=<n>", strprintf(_("Enable automatic wallet backups triggered after each zCmp minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizecmpamount=<n>     " + strprintf(_("Keep N CMP anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Cmp Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Cmp is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Cmp Core is shutting down."));
...
#endif
    // Make sure only a single Cmp process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Cmp Core is probably already running."), strDataDir));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // Cmp: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight()) {
                        RecalculateZCMPMinted();
                        RecalculateZCMPSpent();
                    }
                    RecalculateCMPSupply(1);
                }
                ...
                // Cmp: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Cmp Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Cmp Core to complete") << "\n";
        ...
        bool fEnableZCmpBackups = GetBoolArg("-backupzcmp", true);
        pwalletMain->setZCmpAutoBackups(fEnableZCmpBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeCmpAmount = GetArg("-anonymizecmpamount", 0);
//    if (nAnonymizeCmpAmount > 999999) nAnonymizeCmpAmount = 999999;
//    if (nAnonymizeCmpAmount < 2) nAnonymizeCmpAmount = 2;
    ...
    LogPrintf("Anonymize Cmp Amount %d\n", nAnonymizeCmpAmount);
    ...
    /* Denominations
       ...
       For example:
       1CMP+1000 == (.1CMP+100)*10
       10CMP+10000 == (1CMP+1000)*10
    */
