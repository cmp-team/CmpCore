...
#if defined(NDEBUG)
#error "Cmp cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zCmp spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zCmp spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zCmp spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nPrevBits, int nHeight)
{

    double dDiff;

    if (nHeight <= 150 && Params().NetworkIDString() == CBaseChainParams::MAIN) {
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 100000 * COIN;  //1,450,000
    } else if(nHeight > 1 && nHeight <= 150) { //PoW phase
        nSubsidy = 1 * COIN;
    } else if(nHeight > 150) { //PoS phase
      if(dDiff > 0 && dDiff <= 250) {
        nSubsidy = 5 * COIN;
      } else if(dDiff > 250 && dDiff <= 1500) {
        nSubsidy = 8 * COIN;
      } else if(dDiff > 1500 && dDiff <= 2500) {
        nSubsidy = 10 * COIN;
      } else if(dDiff > 2500 && dDiff <= 5000) {
        nSubsidy = 15 * COIN;
      } else if(dDiff > 5000 && dDiff <= 15000) {
        nSubsidy = 20 * COIN;
      } else if(dDiff > 15000 && dDiff <= 30000) {
        nSubsidy = 23 * COIN;
      } else if(dDiff > 30000 && dDiff <= 45000) {
        nSubsidy = 25 * COIN;
      } else if(dDiff > 45000 && dDiff <= 65000) {
        nSubsidy = 35 * COIN;
      } else if(dDiff > 45000 && dDiff <= 65000) {
        nSubsidy = 45 * COIN;
      } else if(dDiff > 65000 && dDiff <= 150000) {
        nSubsidy = 50 * COIN;
      } else if(dDiff > 150000 && dDiff <= 250000) {
        nSubsidy = 35 * COIN;
      }
...
int nStartTreasuryBlock = 0;
int nTreasuryBlockStep = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Cmp
...
{
    RenameThread("cmp-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZCMPMinted()
{
...
void RecalculateZCMPSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zCMP supply
...
bool RecalculateCMPSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zCmp with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zCmpSpent: %s\n",
...
//              FormatMoney(nFees), FormatMoney(pindex->nMint), FormatMoney(nAmountZerocoinSpent));
    ...
    CAmount nExpectedMint = GetBlockValue(pindex->pprev->nBits, pindex->pprev->nHeight);
    CAmount nExpectedMintNext = GetBlockValue(pindex->pprev->nBits, pindex->pprev->nHeight+1);
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert CMP to zCMP
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (pindexPrev != NULL) {
        ...
        if (nHeight != 0 && !IsInitialBlockDownload()) {
            if (!IsBlockPayeeValid(block, nHeight, nBits)) {
                mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zCmp spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zCmp serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zCmp mints and %d zCmp spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Cmp: We use certain sporks during IBD, so check to see if they are
