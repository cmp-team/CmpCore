...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::CMP, nValue);
...
std::string BlockToString(CBlockIndex* pBlock)
{
    ...
    if (pBlock->nHeight == 0)
        ...
        Generated = GetBlockValue(pBlock->nBits, pBlock->nHeight - 1);
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (cmp.conf).");
            QMessageBox::warning(this, "Cmp Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
