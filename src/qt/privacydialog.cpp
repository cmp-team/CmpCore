                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zCMP ought to be enough for anybody." - Bill Gates, 2017
    ui->zCMPpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzCMPSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzCMP->setEnabled(false);
        ui->pushButtonMintzCMP->setToolTip(tr("zCMP is currently disabled due to maintenance."));

        ui->pushButtonSpendzCMP->setEnabled(false);
        ui->pushButtonSpendzCMP->setToolTip(tr("zCMP is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zCMPpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzCMP_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCMP is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zCMP...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zCMP in ") +
...
void PrivacyDialog::on_pushButtonSpendzCMP_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCMP is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zCMP
        sendzCMP();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zCMP
    sendzCMP();
}

void PrivacyDialog::on_pushButtonZCmpControl_clicked()
{
    ZCmpControlDialog* zCmpControl = new ZCmpControlDialog(this);
    zCmpControl->setModel(walletModel);
    zCmpControl->exec();
}

void PrivacyDialog::setZCmpControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzCmpSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzCMP()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Cmp Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zCMPpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zCMP is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CMP </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zCMP</b>";
    ...
    // use mints from zCmp selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZCmpControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZCmpControlDialog::GetSelectedMints();
    }
    ...
    // Clear zcmp selector in case it was used
    ZCmpControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zCmp Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Cmp, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zCmp Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCMP </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zCMP "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zCMP "));
    ui->labelzCMPAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzCMPSyncStatus->setVisible(fShow);
}
