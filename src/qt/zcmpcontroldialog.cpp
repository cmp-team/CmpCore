...
#include "ui_zcmpcontroldialog.h"
...
std::list<std::string> ZCmpControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZCmpControlDialog::listMints;

ZCmpControlDialog::ZCmpControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZCmpControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZCmpControlDialog::~ZCmpControlDialog()
{
    delete ui;
}

void ZCmpControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCmpControlDialog::updateList()
{
...
void ZCmpControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCmpControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZCmp_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZCmpControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZCmpControlDialog::GetSelectedMints()
{
...
void ZCmpControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
