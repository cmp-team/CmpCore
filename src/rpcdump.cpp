...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"cmpprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"cmpprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
Value importaddress(const Array& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Cmp address or script");
    }
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"cmpaddress\"\n"
            "\nReveals the private key corresponding to 'cmpaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"cmpaddress\"   (string, required) The cmp address for the private key\n"
            "\nResult:\n"
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by Cmp %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
Value bip38encrypt(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38encrypt \"cmpaddress\"\n"
            "\nEncrypts a private key corresponding to 'cmpaddress'.\n"
            "\nArguments:\n"
            "1. \"cmpaddress\"   (string, required) The cmp address for the private key (you must hold the key already)\n"
