...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 CMP at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zCMP send
enum ZerocoinSpendStatus {
    ZCMP_SPEND_OKAY = 0,                            // No error
    ZCMP_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZCMP_WALLET_LOCKED = 2,                         // Wallet was locked
    ZCMP_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZCMP_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZCMP_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZCMP_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZCMP_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZCMP_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZCMP_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZCMP_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZCMP_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZCMP_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZCMP_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZCMP_SPENT_USED_ZCMP = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZCmpAutoBackups(bool fEnabled)
    {
